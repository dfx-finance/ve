...
def test_apply_max_boost(dfx, mock_lp_tokens, voting_escrow, gauge_controller, distributor, veboost_proxy, three_liquidity_gauges_v4, master_account):
    # two random users
    user_0 = accounts[4]
    user_1 = accounts[5]
    ...
    assert additional_vedfx == 4.8436137865170105e+20  # veDFX needed for max boost

    # 4. Deposit needed veDFX (484.361) and test max APR is achieved (~460.18%)
    lock_timestamp = brownie.chain.time()
    deposit_to_ve(dfx, voting_escrow, [user_1],
                  [4.8436137865170105e20], [208], lock_timestamp)
