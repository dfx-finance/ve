...
import addresses
from utils import fastforward_chain as _fastforward_chain, fund_multisig, gas_strategy
...
def test_two_user_apr(dfx, mock_lp_tokens, voting_escrow, gauge_controller, distributor, veboost_proxy, three_liquidity_gauges_v4, master_account):
    ...
    print("2a -- Fast-forward until 5s after epoch 0 starts")
    ...
    gauge_reward_balances = distribute_to_gauges(dfx,
                                                 ...
                                                 master_account,
                                                 {euroc_usdc_gauge: 3232556317728118076767})
    ...
    # 3. Fast-forward until the very end of epoch 0 and claim rewards. Check theoretical vs
    # actually claimed rewards and calculated unboosted APR
    print("3 -- Fast-forward until 5s before epoch 1")
    fastforward_chain(num_weeks=1, delta=-5)
    ...
    print("3 -- Calculated gauge (EUROC/USDC) rewards:",
          theoretical_gauge_rewards / 1e36)
    print(
    ...
    print("4 -- Fast-forward until 10s before epoch 2 (no rewards)")
    ...
    print("5 -- Fast-forward until 10s after epoch 3 (rewards added)")
    fastforward_chain(num_weeks=2, delta=5)
    distributor.updateMiningParameters(
        {'from': master_account, 'gas_price': gas_strategy})
