...
def _distribute_to_all_gauges(distributor, three_liquidity_gauges_v4, account):
    ...
    for reward in rewards_distributed:
        ...
        logs.append([gauge_num, reward["gaugeAddr"], reward["rewardTally"] / 1e18])
    return logs
...
def test_distribute_rewards(
    ...
    for _ in range(10):
        ...
        for gauge in three_liquidity_gauges_v4:
            print(
                f"\t{gauge.name()}",
                gauge_controller.gauge_relative_weight(gauge.address) / 1e18,
            )
        ...
        print(f"Chain time: {chain_time} ({datetime.fromtimestamp(chain_time)})")
        print(
