...
from utils_ve import deposit_to_ve, submit_ve_vote, EMISSION_RATE
...
def test_theoretical_vs_actual(dfx, mock_lp_tokens, voting_escrow, gauge_controller, distributor, veboost_proxy, three_liquidity_gauges_v4, master_account):
    # two random users
    user_0 = accounts[4]
    user_1 = accounts[5]
    ...
    deposit_to_ve(dfx, voting_escrow, [user_0],
                  [1e21], [100], lock_timestamp)
    deposit_to_ve(dfx, voting_escrow, [user_1],
                  [4.35e20], [208], lock_timestamp)
    ...
    # Actual rewards:
    total_gauge_balances = sum([gauge_reward_balances[g]
                                for g in three_liquidity_gauges_v4])
    gauge_balance = gauge_reward_balances[euroc_usdc_gauge]

    assert isclose(theoretical_global_rewards,
                   total_gauge_balances, abs_tol=1e14)
