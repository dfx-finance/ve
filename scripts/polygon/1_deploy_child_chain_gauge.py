...
def deploy_gauge(
    ...
    write_contract(f"{label}PolygonGauge", gauge.address)
    return gauge
...
def deploy_streamer(gauge: RewardsOnlyGauge, label: str) -> ChildChainStreamer:
    ...
    write_contract(f"{label}PolygonStreamer", streamer.address)
    return streamer
...
def deploy_receiver(
    ...
    write_contract(f"{label}PolygonReceiver", receiver.address)
    return receiver
