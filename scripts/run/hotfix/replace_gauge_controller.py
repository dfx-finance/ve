...
output_data = {
    "replacementGaugeController": None,
    "replacementGaugeControllerParams": None,
}


def main():
    dfx_distributor = brownie.interface.IDfxDistributor(DFX_DISTRIBUTOR_ADDRESS)
    # TOGGLE: Only required when running the script step-by-step
    gauge_controller = brownie.interface.IGaugeController(GAUGE_CONTROLLER_ADDRESS)

    # 1. Deploy new GaugeController
    print(f"--- Deploying Gauge Controller contract to {connected_network} ---")
    gauge_controller_params = eth_abi.encode_abi(
        ["address", "address", "address"],
        (addresses.DFX, addresses.VOTE_ESCROW, DEPLOY_ACCT.address),
    ).hex()
    ...
    dfx_distributor.setGaugeController(
        gauge_controller.address, {"from": DEPLOY_ACCT, "gas_price": gas_strategy}
    )
    time.sleep(3)
