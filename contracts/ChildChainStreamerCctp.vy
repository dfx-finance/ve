# @version 0.3.3
"""
@title CCTP Child-Chain Streamer
@author DFX, Curve.Fi, Chainlink
@license MIT
@notice Receives rewards over Chainlink CCIP and evenly streams one or more reward tokens to a single recipient
"""
...
struct Any2EVMMessage:
    messageId: bytes32 # MessageId corresponding to ccipSend on source.
    sourceChainSelector: uint64 # Source chain selector.
    sender: Bytes[32] # abi.decode(sender) if coming from an EVM chain.
    data: Bytes[32] # payload sent in original message.
    destTokenAmounts: DynArray[EVMTokenAmount, 2] #Tokens and their amounts in their destination chain representation.
...
# Determined via separate Solidity script to import interfaces and export
# bytes from type(Interface).interfaceId
IANY2EVMMESSAGERECEIVER_INTERFACE_ID: constant(bytes4) = 0x85572ffb
...
def supportsInterface(interfaceId: bytes4) -> bool:
    return interfaceId == IANY2EVMMESSAGERECEIVER_INTERFACE_ID or interfaceId == IERC165_INTERFACE_ID
...
@internal
def _notify_reward_amount(_token: address):
    """
...
def notify_reward_amount(_token: address):
    self._notify_reward_amount(_token)
...
## CCIP Functions
# override containing custom logic
@internal
def _ccipReceiver(message: Any2EVMMessage):
    self._notify_reward_amount(message.destTokenAmounts[0].token)

@external
def ccipReceive(message: Any2EVMMessage):
    assert msg.sender == i_router, "Invalid router"
    self._ccipReceiver(message)
